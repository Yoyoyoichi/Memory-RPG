...
export async function generateMemoryFloorStory(floorNumber, seedString, salvageCount = 4) {
  ...
  try {
    const floorNumberValue = floorNumber;
    const salvageCountValue = salvageCount;
    const lastChar = String.fromCharCode(64 + salvageCountValue);

    const objectsSchema = Array.from({ length: salvageCountValue }, (_, i) => {
    ...
    }${i === salvageCountValue - 1 ? '' : ','}`;
    }).join('\n');

    // 履歴から完全にサルベージされたオリジナルの指示プロンプト
    const prompt = `あなたはインターネットの過去アーカイブをサルベージする人工知能アナライザーであり、システムログの管理者です。
プレイヤーが「第${floorNumberValue}階層」に到達しました。
このセクターに散らばる未デコードデータ ${salvageCountValue} 個の内容と、このセクターを支配する「観察者NPC」の名前とセリフを、以下のJSONフォーマットで作成してください。
...
{
  "message": "第${floorNumberValue}階層に接続した際に表示するシステムメッセージ（例: 『警告：Geocities - 2002/05/11 のレコードと接続。』など、30文字程度）",
  "npcName": "観察者NPCの名前",
